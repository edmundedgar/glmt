...
- **`posts.jsonl` has no retention policy** — unbounded append-only growth by design (already 1GB+), and nothing prunes or archives it. `live_export_labels.py` is now safe to have this file truncated/rotated out from under it (byte-offset cursor + shrink detection, see "line-count → byte-offset migration" above), so the remaining work is actually deciding *and implementing* a rotation/retention policy, not just making the consumer safe for one. `local_llm_bulk_label.py` and `export_labels.py` don't hold any cursor into this file at all, so they're unaffected by rotation either way.
