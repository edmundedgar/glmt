...
**The V8 heap OOM turned out to be self-limiting, but total process RSS isn't.** `require('v8').getHeapStatistics().heap_size_limit` on this box auto-calculates to ~4.29GB, matching the crash log exactly ("Mark-Compact 4045.6 MB... near heap limit") — so the process was never going to consume the box's full 22GB on its own; V8 kills itself on its own internal ceiling first. But RSS was observed reaching 5.9GB in one crash, past that JS-heap number, meaning real memory (Buffers, native allocations) grows outside what V8's own limit bounds. With no cap configured, that's a genuine cross-process risk: this box was already under real pressure independent of this process (1.4GB free, 1.1GB swap in use when checked), and if total system memory ever got critical, the **kernel's OOM killer can pick any process on the box**, not necessarily this one. Added `MemoryMax=2G` (cgroup-enforced hard ceiling — a leak here now kills only this process, cleanly, and `Restart=always` brings it back) plus `NODE_OPTIONS=--max-old-space-size=1536` (keeps V8's own heap ceiling under the MemoryMax, so the common case is still V8's own loggable `FATAL ERROR` in `journalctl -u labeler-server`, not a silent cgroup SIGKILL).

**The ingester and `live_export_labels.py` got the same treatment for the same reason**, even though neither has a known leak — both have *also* silently died at least once with nothing running to restart them (the ingester's ~3-hour outage that `tools/status.py` caught is the other concrete instance of this exact failure mode, not just the labeler server). Both are safely resumable via their own cursor files, so `Restart=always` is just as low-risk for them as it is here: `deploy/ingester.service`, `deploy/live-export.service`. `local_llm_bulk_label.py` deliberately does **not** get a unit — it's meant to be started/stopped by hand depending on whether the GPU is free for other work, and `Restart=always` would actively fight that usage pattern by restarting it every time it's intentionally stopped.
